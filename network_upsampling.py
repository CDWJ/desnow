...
class deconv_Block(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, output_padding=1):
...
class Stem_Block(nn.Module):
    def __init__(self , in_channels, initial_size):
        ...
        self.conv1 = conv_Block(in_channels, initial_size, 3, stride=2, padding=1)
        self.conv2 = conv_Block(initial_size, initial_size, 3)
        self.conv3 = conv_Block(initial_size, initial_size * 2, 3)

        self.branch1 = conv_Block(initial_size * 2, initial_size * 3, 3, stride=2, padding=1)
...
class reduction_Block_A(nn.Module):
    def __init__(self, in_channels, initial_size):
        ...
        self.branch1 = nn.Sequential(
            ...
            conv_Block(initial_size * 7, initial_size * 8, 3, stride=2, padding=1)
        )

        # self.branch2 = conv_Block(in_channels , 384 , 3 , 2 , 0)
        self.branch2 = conv_Block(in_channels, initial_size * 12, 3, stride=2, padding=1)
...
class reduction_Block_B(nn.Module):
    def __init__(self, in_channels, initial_size):
        ...
        self.branch1 = nn.Sequential(
            ...
            conv_Block(initial_size * 10, initial_size * 10, 3, stride=2, padding=1)
            # conv_Block(320 ,320 ,3 ,2 , 0)  
        )

        self.branch2 = nn.Sequential(
            conv_Block(in_channels, initial_size * 6, 1, padding=0),
            conv_Block(initial_size * 6, initial_size * 6, 3, stride=2, padding=1)
        )
...
class atrous_Conv(nn.Module):
    def __init__(self, inplanes, rate, num_classes=3):
        ...
        self.fc1 = conv_Block(planes, planes // 2, kernel_size=1, stride=1, padding=0)
        self.fc2 = deconv_Block(planes // 2, planes // 4, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.fc3 = deconv_Block(planes // 4, planes // 4, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.fc4 = deconv_Block(planes // 4, planes // 4, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.fc5 = deconv_Block(planes // 4, planes // 8, kernel_size=3, stride=2, padding=1, output_padding=1)
        # self.upsampling = nn.Upsample(scale_factor=2, mode='bicubic')
...
class Snow_model(nn.Module):
    def __init__(self, gamma=4, initial_size=16):
        ...
        self.AE = m_Beta_Model(gamma * (in_feature // 8), 3)
        self.SE = m_Beta_Model(gamma * (in_feature // 8), 1)
        self.RG = m_Beta_Model_sum(gamma * (in_feature // 8), 3)
